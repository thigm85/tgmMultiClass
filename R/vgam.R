...
#' @param data Do not use this parameter unless you know what you are doing. Ideally,
#'  the \code{data} used for training and prediction should be part of \code{resample_indexes} 
#'  to guarantee that comparison between different models are being done with the same dataset.
#'  In case the dataset is not included in \code{resample_indexes} or if you deliberatively want
#'  to use a different dataset, this parameter should contain a data.frame containing the 
#'  response variable and the features used for fitting and predictions.
...
#' 
#' @export
predict_r_vgam <- function(resample_indexes, data = NULL, ...){
